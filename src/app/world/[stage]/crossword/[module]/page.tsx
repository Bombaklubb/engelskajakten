...
import type { StudentData, StageContent, CrosswordModule } from "@/lib/types";

interface Props {
  params: Promise<{ stage: string; module: string }>;
}

export default function CrosswordModulePage({ params }: Props) {
  const { stage: stageId, module: moduleId } = use(params);
  const stage = getStage(stageId);
  const router = useRouter();

  const [student, setStudent] = useState<StudentData | null>(null);
  const [mod, setMod] = useState<CrosswordModule | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/content/${stageId}/content.json`)
      ...
      .then((data: StageContent) => {
        const found = data.crossword?.find((m) => m.id === moduleId);
  ...
  function handleComplete() {
    ...
    if (student) {
      const updated = saveModuleProgress(student, stage!.id, "crossword", mod!.id, pts, true);
      setStudent(updated);
    }
    setShowResult(true);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-sm">🔠 Korsord · {mod.description}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="card">
          <Crossword clues={mod.clues} onComplete={handleComplete} />
        </div>
      </main>
      ...
      {showResult && (
        <ResultModal
          ...
          bonusPoints={0}
          totalCorrect={mod.clues.length}
          totalQuestions={mod.clues.length}
