...
function TidsattackGame({ stageId, stageName, student }: {
  ...
  useEffect(() => {
    if (phase === "result" && awarded === null) {
      const r = addGamePoints("tidsattack", score);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0f0e2e] via-[#1a1840] to-[#0f0e2e]">
      ...
      <div className="flex flex-col items-center justify-center min-h-screen px-4 pt-16">
        <div className="text-center max-w-sm">
          ...
          <Link prefetch={false} href={`/world/${stageId}?tab=spel`} className="mt-4 block text-indigo-300 text-sm hover:text-white transition">← Tillbaka</Link>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col items-center justify-center min-h-screen px-4 pt-16">
        <div className="text-center max-w-sm w-full">
          ...
          <div className="flex gap-3">
            ...
            <Link prefetch={false} href={`/world/${stageId}?tab=spel`} className="flex-1 py-3 bg-white/10 text-white font-bold rounded-xl hover:bg-white/20 transition text-center">Tillbaka</Link>
          </div>
        </div>
      </div>
    </div>
